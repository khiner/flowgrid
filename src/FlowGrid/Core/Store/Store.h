...
const StoreImpl &Get(); // Get a read-only reference to the canonical project store.

nlohmann::json GetJson();
nlohmann::json GetJson(const StoreImpl &);

StoreImpl GetPersistent(); // Get the persistent store from the transient store _without_ ending transient mode.
Patch CheckedSet(const StoreImpl &); // Overwrite the store with the provided store _if it is different_, and return the resulting (potentially empty) patch.
...
Patch CreatePatch(const StoreImpl &before, const StoreImpl &after, const StorePath &base_path = RootPath);

// Create a patch comparing the provided store with the current persistent store.
Patch CreatePatch(const StoreImpl &, const StorePath &base_path = RootPath);
