...
void AddOps(const StoreMap<ValueType> &before, const StoreMap<ValueType> &after, const StorePath &base, PatchOps &ops) {
    diff(
        before,
        after,
        [&](const auto &added) { ops[Path(added, base)].emplace_back(PatchOpType::Add, added.second, std::nullopt); },
        [&](const auto &removed) { ops[Path(removed, base)].emplace_back(PatchOpType::Remove, std::nullopt, removed.second); },
        [&](const auto &o, const auto &n) { ops[Path(o, base)].emplace_back(PatchOpType::Replace, n.second, o.second); }
    );
}
...
void AddOps(const StoreMap<IdPairs> &before, const StoreMap<IdPairs> &after, const StorePath &base, PatchOps &ops) {
    diff(
        ...
        after,
        [&](const auto &added) {
            for (const auto &id_pair : added.second) ops[Path(added, base)].emplace_back(PatchOpType::Insert, SerializeIdPair(id_pair), std::nullopt);
        },
        [&](const auto &removed) {
            for (const auto &id_pair : removed.second) ops[Path(removed, base)].emplace_back(PatchOpType::Erase, std::nullopt, SerializeIdPair(id_pair));
        },
        [&](const auto &o, const auto &n) {
            diff(
                o.second,
                n.second,
                [&](const auto &added) { ops[Path(n, base)].emplace_back(PatchOpType::Insert, SerializeIdPair(added), std::nullopt); },
                [&](const auto &removed) { ops[Path(o, base)].emplace_back(PatchOpType::Erase, std::nullopt, SerializeIdPair(removed)); },
...
void AddOps(const StoreMap<immer::set<u32>> &before, const StoreMap<immer::set<u32>> &after, const StorePath &base, PatchOps &ops) {
    diff(
        ...
        after,
        [&](const auto &added) {
            for (auto v : added.second) ops[Path(added, base)].emplace_back(PatchOpType::Insert, v, std::nullopt);
        },
        [&](const auto &removed) {
            for (auto v : removed.second) ops[Path(removed, base)].emplace_back(PatchOpType::Erase, std::nullopt, v);
        },
        [&](const auto &o, const auto &n) {
            diff(
                o.second,
                n.second,
                [&](auto added) { ops[Path(n, base)].emplace_back(PatchOpType::Insert, added, std::nullopt); },
                [&](auto removed) { ops[Path(o, base)].emplace_back(PatchOpType::Erase, std::nullopt, removed); },
                [](const auto &, const auto &) {} // Change callback required but never called for `immer::set`.
            );
        }
    );
...
template<typename T> void AddOps(const StoreMap<immer::vector<T>> &before, const StoreMap<immer::vector<T>> &after, const StorePath &base, PatchOps &ops) {
    diff(
        before,
        after,
        [&](const auto &added) {
            for (auto v : added.second) ops[Path(added, base)].emplace_back(PatchOpType::PushBack, v, std::nullopt);
        },
        [&](const auto &removed) {
            for (auto v : std::ranges::reverse_view(removed.second)) ops[Path(removed, base)].emplace_back(PatchOpType::PopBack, std::nullopt, v);
        },
        [&](const auto &o, const auto &n) {
            diff(
                o.second,
                ...
                [&](size_t, T added) { ops[Path(n, base)].emplace_back(PatchOpType::PushBack, added, std::nullopt); },
                [&](size_t, T removed) { ops[Path(o, base)].emplace_back(PatchOpType::PopBack, std::nullopt, removed); },
                ...
                [&](size_t i, T o_el, T n_el) { ops[Path(o, base) / std::to_string(i)].emplace_back(PatchOpType::Set, n_el, o_el); }
            );
        }
    );
}
