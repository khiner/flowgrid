...
SoundIoFormat ToSoundIoFormat(const MiniAudio::IoFormat format) {
    switch (format) {
        case MiniAudio::IoFormat_Invalid: return SoundIoFormatInvalid;
        case MiniAudio::IoFormat_Float64NE: return SoundIoFormatFloat64NE;
        case MiniAudio::IoFormat_Float32NE: return SoundIoFormatFloat32NE;
        case MiniAudio::IoFormat_S32NE: return SoundIoFormatS32NE;
        case MiniAudio::IoFormat_S16NE: return SoundIoFormatS16NE;
        default: return SoundIoFormatInvalid;
    }
}
MiniAudio::IoFormat ToAudioFormat(const SoundIoFormat format) {
    switch (format) {
        case SoundIoFormatInvalid: return MiniAudio::IoFormat_Invalid;
        case SoundIoFormatFloat64NE: return MiniAudio::IoFormat_Float64NE;
        case SoundIoFormatFloat32NE: return MiniAudio::IoFormat_Float32NE;
        case SoundIoFormatS32NE: return MiniAudio::IoFormat_S32NE;
        case SoundIoFormatS16NE: return MiniAudio::IoFormat_S16NE;
...
static vector<MiniAudio::IoFormat> SupportedFormats[IO_Count];
...
static void CreateStream(const IO io) {
    ...
    auto prioritized_formats = MiniAudio::PrioritizedDefaultFormats;
    for (const auto &format : prioritized_formats) {
        if (format != MiniAudio::IoFormat_Invalid && soundio_device_supports_format(device, ToSoundIoFormat(format))) {
    ...
    *soundio_format_ptr = ToSoundIoFormat(saved_format != MiniAudio::IoFormat_Invalid ? saved_format : SupportedFormats[io].front());

    auto prioritized_sample_rates = MiniAudio::PrioritizedDefaultSampleRates;
...
static void OpenStream(const IO io) {
    ...
    if (io == IO_In && InStream->layout_error) {
        cerr << "Unable to set " << io << " channel layout: " << soundio_strerror(InStream->layout_error);
    } else if (io == IO_Out && OutStream->layout_error) {
        cerr << "Unable to set " << io << " channel layout: " << soundio_strerror(OutStream->layout_error);
    }
}
