    ...
    switch (param.type) {
        ...
        case Type_HRadioButtons: {
            return label_width_with_spacing +
                ranges::accumulate(NamesAndValues.at(param.zone).names | std::views::transform(CalcRadioChoiceWidth), 0.f) +
                inner_spacing * float(NamesAndValues.size());
        }
        case Type_Menu: {
            return label_width_with_spacing + std::ranges::max(NamesAndValues.at(param.zone).names | std::views::transform([](const string &choice_name) {
        ...
        case Type_VRadioButtons: return max(std::ranges::max(NamesAndValues.at(param.zone).names | std::views::transform(CalcRadioChoiceWidth)), label_width);
...
void FaustParamsUI::DrawUiItem(const FaustParam &param, const char *label, const float suggested_height) const {
    ...
    if (type == Type_None || type == Type_TGroup || type == Type_HGroup || type == Type_VGroup) {
        ...
        if (type == Type_Button) {
            ...
            if (RadioButtons(param.label.c_str(), &value, NamesAndValues.at(param.zone), flags, justify)) *param.zone = Real(value);
        } else if (type == Type_Menu) {
            auto value = float(*param.zone);
            const auto &names_and_values = NamesAndValues.at(param.zone);
...
void FaustParamsUI::Render() const {
    DrawUiItem(UiParam, "", GetContentRegionAvail().y);
