...
const char *TSReadText(void *payload, uint32_t byte_index, TSPoint position, uint32_t *bytes_read) {
    ...
    const auto *editor = static_cast<TextBuffer *>(payload);
...
TextBuffer::TextBuffer(std::string_view text, LanguageID language_id)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
    SetLanguage(language_id);
    SetText(string(text));
    Commit();
}
TextBuffer::TextBuffer(const fs::path &file_path)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
...
void TextBuffer::ApplyEdits() {
    Syntax->ApplyEdits(Edits);
    Edits.clear();
}

string TextBuffer::GetSyntaxTreeSExp() const { return Syntax->GetSExp(); }

const string &TextBuffer::GetLanguageName() const { return Languages.Get(LanguageId).Name; }
const string &TextBuffer::GetLanguageFileExtensionsFilter() { return Languages.AllFileExtensionsFilter; }

const TextBuffer::PaletteT &TextBuffer::GetPalette() const {
    switch (PaletteId) {
...
void TextBuffer::SetPalette(PaletteIdT palette_id) { PaletteId = palette_id; }

void TextBuffer::SetLanguage(LanguageID language_id) {
...
void TextBuffer::SetNumTabSpaces(uint tab_size) { NumTabSpaces = std::clamp(tab_size, 1u, 8u); }
void TextBuffer::SetLineSpacing(float line_spacing) { LineSpacing = std::clamp(line_spacing, 1.f, 2.f); }
...
bool TextBuffer::CanPaste() const { return !ReadOnly && ImGui::GetClipboardText() != nullptr; }
...
void TextBuffer::SetFilePath(const fs::path &file_path) {
...
string TextBuffer::GetText(LineChar start, LineChar end) const {
    if (end <= start) return "";
...
bool TextBuffer::Cursors::AnyRanged() const {
    return any_of(Cursors, [](const auto &c) { return c.IsRange(); });
}
bool TextBuffer::Cursors::AllRanged() const {
    return all_of(Cursors, [](const auto &c) { return c.IsRange(); });
}
bool TextBuffer::Cursors::AnyMultiline() const {
    return any_of(Cursors, [](const auto &c) { return c.IsMultiline(); });
}
bool TextBuffer::Cursors::AnyEdited() const {
    return any_of(Cursors, [](const auto &c) { return c.IsEdited(); });
}

void TextBuffer::Cursors::Add() {
    Cursors.push_back({});
    LastAddedIndex = size() - 1;
}
void TextBuffer::Cursors::Reset() {
    Cursors.clear();
    Add();
}
void TextBuffer::Cursors::SortAndMerge() {
    if (size() <= 1) return;
...
std::optional<TextBuffer::Cursor> TextBuffer::Cursors::GetEditedCursor() {
    if (!AnyEdited()) return {};
...
void TextBuffer::InsertTextAtCursor(Lines text, Cursor &c) {
    if (!text.empty()) c.Set(InsertText(text, c.Min()));
}

void TextBuffer::LinesIter::MoveRight() {
    if (LC == End) return;
...
void TextBuffer::LinesIter::MoveLeft() {
    if (LC == Begin) return;
...
void TextBuffer::MoveCharIndexAndColumn(const Line &line, uint &ci, uint &column) const {
    const char ch = line[ci];
    ci += UTF8CharLength(ch);
    column = ch == '\t' ? NextTabstop(column, NumTabSpaces) : column + 1;
}

uint TextBuffer::NumStartingSpaceColumns(uint li) const {
...
void TextBuffer::Cursor::MoveLines(const TextBuffer &editor, int amount, bool select, bool move_start, bool move_end) {
...
void TextBuffer::Cursor::MoveChar(const TextBuffer &editor, bool right, bool select, bool is_word_mode) {
...
void TextBuffer::Cursors::MoveLines(const TextBuffer &editor, int amount, bool select, bool move_start, bool move_end) {
    for (auto &c : Cursors) c.MoveLines(editor, amount, select, move_start, move_end);
}
void TextBuffer::Cursors::MoveChar(const TextBuffer &editor, bool right, bool select, bool is_word_mode) {
...
void TextBuffer::Cursors::MoveTop(bool select) { Cursors.back().Set({0, 0}, !select); }
void TextBuffer::Cursors::MoveBottom(const TextBuffer &editor, bool select) { Cursors.back().Set(editor.LineMaxLC(editor.LineCount() - 1), !select); }
void TextBuffer::Cursors::MoveStart(bool select) {
    for (auto &c : Cursors) c.Set({c.Line(), 0}, !select);
}
void TextBuffer::Cursors::MoveEnd(const TextBuffer &editor, bool select) {
    for (auto &c : Cursors) c.Set(editor.LineMaxLC(c.Line()), !select);
}

void TextBuffer::EnterChar(ImWchar ch, bool is_shift) {
...
void TextBuffer::Backspace(bool is_word_mode) {
    BeforeCursors = Cursors;
...
void TextBuffer::Delete(bool is_word_mode) {
    BeforeCursors = Cursors;
    if (!Cursors.AnyRanged()) {
...
void TextBuffer::SetSelection(LineChar start, LineChar end, Cursor &c) {
    const LineChar min_lc{0, 0}, max_lc{LineMaxLC(Text.size() - 1)};
    c.Set(std::clamp(start, min_lc, max_lc), std::clamp(end, min_lc, max_lc));
}

void TextBuffer::AddCursorForNextOccurrence(bool case_sensitive) {
...
std::optional<TextBuffer::Cursor> TextBuffer::FindNextOccurrence(const string &text, LineChar start, bool case_sensitive) {
...
std::optional<TextBuffer::Cursor> TextBuffer::FindMatchingBrackets(const Cursor &c) {
...
void TextBuffer::ChangeCurrentLinesIndentation(bool increase) {
    BeforeCursors = Cursors;
...
void TextBuffer::SwapLines(uint li1, uint li2) {
...
void TextBuffer::MoveCurrentLines(bool up) {
    BeforeCursors = Cursors;
...
void TextBuffer::ToggleLineComment() {
...
void TextBuffer::RemoveCurrentLines() {
    BeforeCursors = Cursors;
    for (auto &c : reverse_view(Cursors)) DeleteSelection(c);
    Cursors.MoveStart();
    Cursors.SortAndMerge();
...
TextBuffer::Coords TextBuffer::ScreenPosToCoords(const ImVec2 &screen_pos, bool *is_over_li) const {
...
TextBuffer::LineChar TextBuffer::FindWordBoundary(LineChar from, bool is_start) const {
...
uint TextBuffer::GetCharIndex(const Line &line, uint column) const {
    uint ci = 0;
    for (uint column_i = 0; ci < line.size() && column_i < column;) MoveCharIndexAndColumn(line, ci, column_i);
    return ci;
}

uint TextBuffer::GetColumn(const Line &line, uint ci) const {
    uint column = 0;
    for (uint ci_i = 0; ci_i < ci && ci_i < line.size();) MoveCharIndexAndColumn(line, ci_i, column);
    return column;
}

uint TextBuffer::GetFirstVisibleCharIndex(const Line &line, uint first_visible_column) const {
    uint ci = 0, column = 0;
...
uint TextBuffer::GetLineMaxColumn(const Line &line) const {
    uint column = 0;
    for (uint ci = 0; ci < line.size();) MoveCharIndexAndColumn(line, ci, column);
    return column;
}
uint TextBuffer::GetLineMaxColumn(const Line &line, uint limit) const {
    uint column = 0;
    for (uint ci = 0; ci < line.size() && column < limit;) MoveCharIndexAndColumn(line, ci, column);
    return column;
}

TextBuffer::LineChar TextBuffer::InsertText(Lines text, LineChar at, bool update_cursors) {
...
void TextBuffer::DeleteRange(LineChar start, LineChar end, bool update_cursors, const Cursor *exclude_cursor) {
    if (end <= start) return;
...
void TextBuffer::DeleteSelection(Cursor &c) {
    if (!c.IsRange()) return;
...
uint TextBuffer::ToByteIndex(LineChar lc) const {
...
*/
TextBufferStyle::CharStyle TSConfig::FindStyleByCaptureName(const std::string &capture_name) const {
...
void TextBuffer::HandleKeyboardInputs() {
...
void TextBuffer::CreateHoveredNode(uint byte_index) {
    DestroyHoveredNode();
...
void TextBuffer::DestroyHoveredNode() {
    if (HoveredNode) {
...
void TextBuffer::HandleMouseInputs() {
...
bool TextBuffer::Render(bool is_parent_focused) {
...
const TextBuffer::PaletteT TextBuffer::DarkPalette = {{
    0xffe4dfdc, // Default
...
const TextBuffer::PaletteT TextBuffer::MarianaPalette = {{
    0xffffffff, // Default
...
const TextBuffer::PaletteT TextBuffer::LightPalette = {{
    0xff404040, // Default
...
const TextBuffer::PaletteT TextBuffer::RetroBluePalette = {{
    0xff00ffff, // Default
