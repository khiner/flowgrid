...
struct TextBuffer {
    TextBuffer(std::string_view text = "", LanguageID language_id = LanguageID::None);
    TextBuffer(const fs::path &);
    ~TextBuffer();
...
private:
    ...
    struct Cursor {
        ...
        uint GetStartColumn(const TextBuffer &editor) {
            if (!StartColumn) StartColumn = editor.GetColumn(Start);
            return *StartColumn;
        }
        uint GetEndColumn(const TextBuffer &editor) {
            if (!EndColumn) EndColumn = editor.GetColumn(End);
            return *EndColumn;
        }
        Coords GetStartCoords(const TextBuffer &editor) { return {Start.L, GetStartColumn(editor)}; }
        Coords GetEndCoords(const TextBuffer &editor) { return {End.L, GetEndColumn(editor)}; }
        ...
        void MoveChar(const TextBuffer &, bool right = true, bool select = false, bool is_word_mode = false);
        void MoveLines(const TextBuffer &, int amount = 1, bool select = false, bool move_start = false, bool move_end = true);
    ...
    struct Cursors {
        ...
        void MoveBottom(const TextBuffer &, bool select = false);
        void MoveStart(bool select = false);
        void MoveEnd(const TextBuffer &, bool select = false);
    ...
    float TextStart{20}; // Position (in pixels) where a code line starts relative to the left of the TextBuffer.
    uint NumTabSpaces{4};
    uint LeftMargin{10};
    ImVec2 CharAdvance;
