...
const char *TSReadText(void *payload, uint32_t byte_index, TSPoint position, uint32_t *bytes_read) {
    ...
    const auto *editor = static_cast<TextBufferImpl *>(payload);
...
TextBufferImpl::TextBufferImpl(std::string_view text, LanguageID language_id)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
    SetLanguage(language_id);
    SetText(string(text));
    Commit();
}
TextBufferImpl::TextBufferImpl(const fs::path &file_path)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
    OpenFile(file_path);
    Commit();
}
...
void TextBufferImpl::ApplyEdits() {
    Syntax->ApplyEdits(Edits);
    Edits.clear();
}

string TextBufferImpl::GetSyntaxTreeSExp() const { return Syntax->GetSExp(); }

const string &TextBufferImpl::GetLanguageName() const { return Languages.Get(LanguageId).Name; }
const string &TextBufferImpl::GetLanguageFileExtensionsFilter() const { return Languages.AllFileExtensionsFilter; }

const TextBufferImpl::PaletteT &TextBufferImpl::GetPalette() const {
    switch (PaletteId) {
...
void TextBufferImpl::SetPalette(TextBufferPaletteIdT palette_id) { PaletteId = palette_id; }

void TextBufferImpl::SetLanguage(LanguageID language_id) {
...
void TextBufferImpl::SetNumTabSpaces(uint tab_size) { NumTabSpaces = std::clamp(tab_size, 1u, 8u); }
void TextBufferImpl::SetLineSpacing(float line_spacing) { LineSpacing = std::clamp(line_spacing, 1.f, 2.f); }

void TextBufferImpl::SelectAll() {
    Cursors.Reset();
    Cursors.MoveTop();
    Cursors.MoveBottom(*this, true);
}

void TextBufferImpl::Copy() {
...
bool TextBufferImpl::CanPaste() const { return !ReadOnly && ImGui::GetClipboardText() != nullptr; }

void TextBufferImpl::Paste() {
...
void TextBufferImpl::SetText(const string &text) {
...
void TextBufferImpl::OpenFile(const fs::path &file_path) {
    SetFilePath(file_path);
    SetText(FileIO::read(file_path));
}

void TextBufferImpl::SetFilePath(const fs::path &file_path) {
...
string TextBufferImpl::GetText(LineChar start, LineChar end) const {
    if (end <= start) return "";
...
bool TextBufferImpl::Cursors::AnyRanged() const {
    return any_of(Cursors, [](const auto &c) { return c.IsRange(); });
}
bool TextBufferImpl::Cursors::AllRanged() const {
    return all_of(Cursors, [](const auto &c) { return c.IsRange(); });
}
bool TextBufferImpl::Cursors::AnyMultiline() const {
    return any_of(Cursors, [](const auto &c) { return c.IsMultiline(); });
}
bool TextBufferImpl::Cursors::AnyEdited() const {
    return any_of(Cursors, [](const auto &c) { return c.IsEdited(); });
}

void TextBufferImpl::Cursors::Add() {
    Cursors.push_back({});
    LastAddedIndex = size() - 1;
}
void TextBufferImpl::Cursors::Reset() {
    Cursors.clear();
    Add();
}
void TextBufferImpl::Cursors::SortAndMerge() {
    if (size() <= 1) return;
...
std::optional<TextBufferImpl::Cursor> TextBufferImpl::Cursors::GetEditedCursor() {
    if (!AnyEdited()) return {};
...
void TextBufferImpl::InsertTextAtCursor(Lines text, Cursor &c) {
    if (!text.empty()) c.Set(InsertText(text, c.Min()));
}

void TextBufferImpl::LinesIter::MoveRight() {
    if (LC == End) return;
...
void TextBufferImpl::LinesIter::MoveLeft() {
    if (LC == Begin) return;
...
void TextBufferImpl::MoveCharIndexAndColumn(const Line &line, uint &ci, uint &column) const {
    const char ch = line[ci];
    ci += UTF8CharLength(ch);
    column = ch == '\t' ? NextTabstop(column, NumTabSpaces) : column + 1;
}

uint TextBufferImpl::NumStartingSpaceColumns(uint li) const {
...
void TextBufferImpl::EnterChar(ImWchar ch, bool is_shift) {
...
void TextBufferImpl::Backspace(bool is_word_mode) {
    BeforeCursors = Cursors;
...
void TextBufferImpl::Delete(bool is_word_mode) {
    BeforeCursors = Cursors;
    if (!Cursors.AnyRanged()) {
...
void TextBufferImpl::SetSelection(LineChar start, LineChar end, Cursor &c) {
    const LineChar min_lc{0, 0}, max_lc{LineMaxLC(Text.size() - 1)};
    c.Set(std::clamp(start, min_lc, max_lc), std::clamp(end, min_lc, max_lc));
}

void TextBufferImpl::AddCursorForNextOccurrence(bool case_sensitive) {
...
std::optional<TextBufferImpl::Cursor> TextBufferImpl::FindNextOccurrence(const string &text, LineChar start, bool case_sensitive) {
...
std::optional<TextBufferImpl::Cursor> TextBufferImpl::FindMatchingBrackets(const Cursor &c) {
...
void TextBufferImpl::ChangeCurrentLinesIndentation(bool increase) {
    BeforeCursors = Cursors;
...
void TextBufferImpl::SwapLines(uint li1, uint li2) {
...
void TextBufferImpl::MoveCurrentLines(bool up) {
    BeforeCursors = Cursors;
...
void TextBufferImpl::ToggleLineComment() {
...
void TextBufferImpl::RemoveCurrentLines() {
    BeforeCursors = Cursors;
    for (auto &c : reverse_view(Cursors)) DeleteSelection(c);
    Cursors.MoveStart();
    Cursors.SortAndMerge();
...
TextBufferImpl::Coords TextBufferImpl::ScreenPosToCoords(const ImVec2 &screen_pos, bool *is_over_li) const {
...
TextBufferImpl::LineChar TextBufferImpl::FindWordBoundary(LineChar from, bool is_start) const {
...
uint TextBufferImpl::GetCharIndex(const Line &line, uint column) const {
    uint ci = 0;
    for (uint column_i = 0; ci < line.size() && column_i < column;) MoveCharIndexAndColumn(line, ci, column_i);
    return ci;
}

uint TextBufferImpl::GetColumn(const Line &line, uint ci) const {
    uint column = 0;
    for (uint ci_i = 0; ci_i < ci && ci_i < line.size();) MoveCharIndexAndColumn(line, ci_i, column);
    return column;
}

uint TextBufferImpl::GetFirstVisibleCharIndex(const Line &line, uint first_visible_column) const {
    uint ci = 0, column = 0;
...
uint TextBufferImpl::GetLineMaxColumn(const Line &line) const {
    uint column = 0;
    for (uint ci = 0; ci < line.size();) MoveCharIndexAndColumn(line, ci, column);
    return column;
}
uint TextBufferImpl::GetLineMaxColumn(const Line &line, uint limit) const {
    uint column = 0;
    for (uint ci = 0; ci < line.size() && column < limit;) MoveCharIndexAndColumn(line, ci, column);
    return column;
}

TextBufferImpl::LineChar TextBufferImpl::InsertText(Lines text, LineChar at, bool update_cursors) {
...
void TextBufferImpl::DeleteRange(LineChar start, LineChar end, bool update_cursors, const Cursor *exclude_cursor) {
    if (end <= start) return;
...
void TextBufferImpl::DeleteSelection(Cursor &c) {
    if (!c.IsRange()) return;
...
uint TextBufferImpl::ToByteIndex(LineChar lc) const {
...
*/
TextBufferImplStyle::CharStyle TSConfig::FindStyleByCaptureName(const std::string &capture_name) const {
...
void TextBufferImpl::HandleKeyboardInputs() {
...
void TextBufferImpl::CreateHoveredNode(uint byte_index) {
    DestroyHoveredNode();
...
void TextBufferImpl::DestroyHoveredNode() {
    if (HoveredNode) {
...
void TextBufferImpl::HandleMouseInputs() {
