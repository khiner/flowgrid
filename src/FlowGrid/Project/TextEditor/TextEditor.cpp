...
struct SyntaxTree {
    ...
    void SetLanguage(LanguageID language_id) {
        ...
        for (uint i = 0; i < capture_count; ++i) {
            ...
            StyleByCaptureId[i] = Config.FindStyleByCaptureName(string(capture_name, length));
        }
...
TextEditor::TextEditor(std::string_view text, LanguageID language_id)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
    SetLanguage(language_id);
    SetText(string(text));
    Commit();
}
TextEditor::TextEditor(const fs::path &file_path)
    : Syntax(std::make_unique<SyntaxTree>(TSInput{this, TSReadText, TSInputEncodingUTF8})) {
