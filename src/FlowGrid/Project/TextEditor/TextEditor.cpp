...
void TextEditor::Paste() {
    ...
    if (clip_text.length() > 0) {
        ...
        for (int c = State.CurrentCursor; c > -1; c--) {
            ...
            u.Operations.emplace_back(insert_text, end, GetCursorPosition(c), UndoOperationType::Add);
        }
...
void TextEditor::MoveCoords(Coordinates &coords, MoveDirection direction, bool is_word_mode, int line_count) const {
    int ci = GetCharacterIndexR(coords);
    int lindex = coords.L;
    switch (direction) {
        case MoveDirection::Right:
            if (ci >= int(Lines[lindex].size())) {
                ...
                Move(lindex, ci);
                int one_step_right_column = GetCharacterColumn(lindex, ci);
                if (is_word_mode) {
        ...
        case MoveDirection::Left:
            if (ci == 0) {
                ...
                Move(lindex, ci, true);
                coords.C = GetCharacterColumn(lindex, ci);
...
void TextEditor::MoveTop(bool select) {
    SetCursorPosition({0, 0}, State.Cursors[State.CurrentCursor], !select);
}
...
void TextEditor::EnterCharacter(ImWchar character, bool is_shift) {
    ...
    for (int c = State.CurrentCursor; c > -1; c--) {
        ...
        if (character == '\n') {
            ...
            SetCursorPosition({coord.L + 1, GetCharacterColumn(coord.L + 1, int(whitespace_size))}, cursor);
        } else {
            char buf[5];
            ...
            SetCursorPosition({coord.L, GetCharacterColumn(coord.L, ci + glyphs.size())}, cursor);
        }
...
void TextEditor::SetSelection(Coordinates start, Coordinates end, int c) {
    SetSelection(start, end, State.Cursors[c == -1 ? State.CurrentCursor : c]);
}
...
void TextEditor::RemoveCurrentLines() {
    ...
    for (int c = State.CurrentCursor; c > -1; c--) {
        ...
        if (to_delete_start.L != to_delete_end.L) {
            ...
            for (int other_c = 0; other_c <= State.CurrentCursor; other_c++) {
...
int TextEditor::GetCharacterIndexL(const Coordinates &coords) const {
    ...
    for (int c = 0; c < coords.C && i < int(line.size()); c++) {
        if (line[i].Char == '\t') {
...
std::unordered_map<int, int> TextEditor::BeforeLineChanged(int li, int column, int char_count, bool is_deleted) {
    ...
    for (int c = 0; c <= State.CurrentCursor; c++) {
        ...
        if (cursor.InteractiveEnd.L == li && cursor.InteractiveEnd.C > column && !cursor.HasSelection()) {
            adjusted_ci_for_cursor[c] = GetCharacterIndexR({li, cursor.InteractiveEnd.C}) + (is_deleted ? -char_count : char_count);
        }
    }
...
void TextEditor::AfterLineChanged(int li, std::unordered_map<int, int> &&adjusted_ci_for_cursor) {
    for (const auto &[c, ci] : adjusted_ci_for_cursor) {
        auto &cursor = State.Cursors[c];
        SetCursorPosition({li, GetCharacterColumn(li, ci)}, cursor);
    }
}
...
void TextEditor::HandleMouseInputs() {
    ...
    if (ImGui::IsWindowHovered()) {
        ...
        if (!shift && !alt) {
            ...
            auto &current_cursor = State.Cursors[State.CurrentCursor];
            SetCursorPosition(SanitizeCoordinates(new_selection), current_cursor, false);
        }
    }
}
...
void TextEditor::Render(bool is_parent_focused) {
            ...
            if (cursor_coords_in_this_line.size() > 0) {
                ...
                if (ImGui::IsWindowFocused() || is_parent_focused) {
                    for (const auto &cursor_coords : cursor_coords_in_this_line) {
                        float width = 1.0f;
                        const int ci = GetCharacterIndexR(cursor_coords);
    ...
    if (LastEnsureCursorVisible > -1) {
        ...
        for (int i = 0; i < (LastEnsureCursorVisibleStartToo ? 2 : 1); i++) {
            ...
            const auto target = GetCursorPosition(LastEnsureCursorVisible, i); // Cursor selection end or start
